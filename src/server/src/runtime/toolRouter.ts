...
/** 工具分类映射：根据 apiName 前缀确定类别 */
const CATEGORY_PATTERNS: [string, ToolCategory][] = [
  ["secops_wazuh_", "wazuh-platform"],
  ["secops_shuffle_", "shuffle-soar"],
...
export class ToolRouter {
  ...
  build(registry: ToolRegistry): void {
    ...
    for (const m of manifests) {
      ...
      // 报告工具
      if (m.skillPackId === "secops-reports") {
        this.categoryMap.get("reporting")!.push(m.id);
        continue;
      }
      // 动作工具
