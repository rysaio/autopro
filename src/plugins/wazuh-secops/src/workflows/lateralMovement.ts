...
import type { WazuhAlertSearchInput, WazuhIndexerClient, DemoWazuhIndexerClient } from "../core/indexerClient.js";
...
export async function agentAlertTimeline(indexer: WazuhIndexerClientLike, request: TimelineRequest): Promise<Record<string, unknown>> {
...
export async function ipActivityTimeline(indexer: WazuhIndexerClientLike, request: TimelineRequest): Promise<Record<string, unknown>> {
  const response = await indexer.searchAlerts(toAlertSearchInput(request));
  const alerts = alertHits(response).map(enrichedAlertSummary);
  return {
...
export async function networkServiceFind(client: WazuhClientLike, request: ServiceFinderRequest): Promise<Record<string, unknown>> {
...
export async function hostNeighbors(
  client: WazuhClientLike,
  indexer: WazuhIndexerClientLike,
...
export async function lateralPathSummary(indexer: WazuhIndexerClientLike, request: LateralPathRequest): Promise<Record<string, unknown>> {
...
async function collectAgentAddresses(client: WazuhClientLike, agentId: string): Promise<{
...
async function collectAgentIpMap(client: WazuhClientLike, limit: number): Promise<Map<string, Record<string, unknown>>> {
