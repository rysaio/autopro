...
Date: 2026-06-19
Status: accepted (durable-store decision superseded by [2026-06-27 Embedded PGlite Durable Sessions](2026-06-27-embedded-pglite-durable-sessions.md); control-boundary decisions still in force)
...
- A durable store is the source of truth for sessions, runs, messages, tool
  invocations, recoverable guidance, artifacts, state markers, pending
  approvals, and audit events. (The engine was external Postgres at the time of
  this ADR; superseded — see the 2026-06-27 ADR for embedded PGlite.)
...
- Completion evidence must exercise real durable recovery, not skipped tests.
  (The 2026-06-27 ADR moves this to an in-process PGlite restart round-trip.)
...
- Durable recovery and manual Web rendering checks remain required before
  claiming final acceptance of the full control-boundary goal. (Durable
  recovery is now covered by the embedded PGlite test; see the 2026-06-27 ADR.)
