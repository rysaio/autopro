...
Node.js 建议使用 24 LTS。首次运行前，将 `.env.example` 复制为 `.env` 或 `.env.local`，并填写模型配置，例如 `MODEL_API_KEY`、`MODEL_BASE_URL` 和 `SECOPS_MODEL`。实际使用的配置文件为 `src/secops.config.json`，也可以通过环境变量覆盖。
...
这种方式不会读取或修改 `src/server/runtime/pgdata`。若前端也需要连接该端口，可设置 `VITE_API_BASE_URL=http://127.0.0.1:4327` 后重新启动 Vite。
