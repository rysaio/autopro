...
- 全局使用冷白、雾蓝灰表面和单一标准蓝强调色。
- 侧栏改为浅色工作台，不再使用深色侧栏。
- 对话区为无边框主画布，内容宽度集中在约 840px。
- 用户消息靠右并使用浅灰气泡；Agent 消息使用无气泡正文。
- 输入框为居中的独立操作面，宽度与对话正文对齐。
- Tool 消息默认折叠，审批和引导状态仍保留明确的结构与操作。
- 知识图谱颜色同步为中性灰和低饱和辅助色，避免与全局强调色竞争。
